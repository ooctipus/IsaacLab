...
.. list-table::
   ...
   * - ``collision_paths``
     - Exact plan-owned roots whose cfg declares ``collision_group=-1``.
   * - ``env_template``
     - Path template used for environment roots and namespace expansion.

``clone_mask``, ``env_ids``, and ``positions`` live on one device. Environment ids are unique and
nonnegative; mask column ``j`` and position row ``j`` both describe ``env_ids[j]``.
...
.. code-block:: text
   ...
   OvPhysxReplicateContext  registers OvPhysX clone transforms
...
Normal scene construction uses the lifecycle above. Raw :func:`~isaaclab.cloner.usd_replicate`
remains available for low-level tooling and focused tests:
...
Tools that already own a routed plan can execute the same public context contract used by the
lifecycle. These are backend alternatives, not a sequence to run on one simulation:
...
.. code-block:: python
   ...
   # USD, or USD followed by PhysX when native PhysX consumes authored destinations.
   cloner.UsdReplicateContext(stage).replicate(plan)
   physx_context = PhysxReplicateContext(stage)
   physx_context.replicate(plan)

   # Newton and OvPhysX instead receive the owning simulation.
   NewtonReplicateContext(sim).replicate(plan)
   OvPhysxReplicateContext(sim).replicate(plan)

USD must author destination topology before native PhysX consumes it. The plan remains each
context's only replication input; constructors receive only their native runtime. Retain a manually
constructed PhysX context while its registration is in use, then call ``physx_context.clear()`` at
