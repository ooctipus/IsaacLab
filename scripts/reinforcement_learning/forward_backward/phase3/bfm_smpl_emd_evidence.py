...
"""bfm-ab-20260805 HEAD port of :mod:`smpl_cmu_emd_evidence` (native held-out SMPL-CMU EMD).

Campaign port (pre-registered deviation iii in the F ledger row bfm-ab-20260805):
the original script predates three HEAD changes and refuses to run at the campaign
code state — (a) the raw ``cmu`` preset token no longer selects the native HumEnv
oracle corpus, so the evaluation environment resolves the campaign-scoped
``humenv_cmu_control`` registration (identifier ``cmu_humenv_smpl``, byte-identical
split pins to the oracle registration; the oracle purpose gate refuses live
environment builds by design); (b) ``reference_artifact_root`` was renamed
``target_artifact_root``; (c) the expert clock moved under ``expert.clock``.
Protocol, metric, and identity fields are unchanged from the original.
"""
...
def _environment_tokens() -> frozenset[str]:
    """Native evaluation axes with the training-control HumEnv registration selected."""
    return frozenset((motion_environment_axes(_PROFILE) - {"cmu"}) | {"humenv_cmu_control"})
