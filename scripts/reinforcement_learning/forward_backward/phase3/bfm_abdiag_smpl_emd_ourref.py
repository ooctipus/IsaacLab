...
"""bfm-abdiag-20260805 OWN-REFERENCE variant of :mod:`bfm_smpl_emd_evidence`.

Eval-side diagnosis script (F ledger row bfm-abdiag-20260805): identical faithful
protocol (reset to source frame zero, deterministic actions, exact uniform transport
over the first 214 observation values, clip-safe 8-frame contexts, env/eval seeds),
but the evaluation references come from OUR campaign v5-dump deployment instead of
the original HumEnv corpus: profile ``smpl_cmu_retarget`` (token ``cmu_retarget``,
identifier ``cmu_smpl_retarget_dump_v5``), evaluation split = 168 accepted clips /
76,250 frames at the converter 30 Hz clock. Together with the archived
original-reference G5 numbers this fills the (train-domain x eval-domain) 2x2.
Nothing tuned; production untouched.
"""
...
def _environment_tokens() -> frozenset[str]:
    """Native evaluation axes with the campaign v5-dump registration selected."""
    return frozenset(motion_environment_axes(_PROFILE))
...
def _run(args: argparse.Namespace, request: _EvidenceRequest) -> tuple[dict[str, object], ...]:
    ...
    from isaaclab_tasks.core.multi_task.motion.data.sources import RetargetDumpV5Clips
    ...
    table_cfg.target_artifact_root = str(args.reference_artifact_root.expanduser().resolve())
    ...
    environment_dependency_identity = motion_environment_dependency_identity(
        preset=_PROFILE,
        cfg=cfg,
        importer_type=RetargetDumpV5Clips,
    ...
    runner_selection = _environment_tokens() | motion_runner_axes(_PROFILE)
