...
if not _MISSING_MODULES and not _ovrtx_below_0_3():
    core_env_tests = Path(__file__).resolve().parents[2] / "isaaclab" / "test" / "envs"
    sys.path.insert(0, str(core_env_tests))
...
def test_visual_color_contract_ovrtx():
    """Bucket writes propagate to all envs, buckets are distinct and independent on the OVRTX renderer."""
...
def test_per_env_visual_color_contract_ovrtx():
    ...
    assert_per_env_color_contract(OVRTXRendererCfg(), physics_cfg=NewtonCfg(solver_cfg=MJWarpSolverCfg()))
