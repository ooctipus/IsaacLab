...
def vision_forward(
    self, obs: dict[str, torch.Tensor], group2encoder: dict[str, str], batch_size: int, device: str = "cpu"
) -> TensorDict:
    enc = self.encoders.to(device)
...
def projector_forward(
    ...
    for projector_key, feature_keys in projector_features.items():
        ...
        prediction_targets = torch.cat(
            [obs_batch[tgt_key] for tgt_key in projector_prediction_targets[projector_key]], dim=1
        )
