...
import isaaclab.cloner.clone_plan as clone_plan_module
import isaaclab.cloner.replicate_session as replicate_session
import isaaclab.sim as sim_utils
from isaaclab.assets import ArticulationCfg, AssetBaseCfg
from isaaclab.cloner import ClonePlan
from isaaclab.cloner.clone_plan import _asset_inventory, _make_clone_plan
...
def _simulation(registry=None, roles=None, stage=None):
    simulation = SimpleNamespace(
        ...
        _clone_models_initialized=False,
...
def test_cfgs_and_consumers_do_not_own_clone_lifecycle():
    ...
    assert not hasattr(clone_plan_module, "_plan_cfgs")
    assert "_pending_clone_model_contexts" not in inspect.getsource(SimulationContext)
    planner_source = inspect.getsource(replicate_session)
    assert "renderer_cfg" not in planner_source
    assert "_get_visualizer_cfgs" not in planner_source
...
@pytest.mark.parametrize(
    ("up_axis", "expected"),
    [
        ("z", [[1.0, -1.0, 0.0], [1.0, 1.0, 0.0], [-1.0, -1.0, 0.0], [-1.0, 1.0, 0.0]]),
        ("y", [[1.0, 0.0, -1.0], [1.0, 0.0, 1.0], [-1.0, 0.0, -1.0], [-1.0, 0.0, 1.0]]),
        ("x", [[0.0, 1.0, -1.0], [0.0, 1.0, 1.0], [0.0, -1.0, -1.0], [0.0, -1.0, 1.0]]),
    ],
)
def test_grid_positions(up_axis, expected):
    """The public helper returns the clone layout without redundant orientations."""
    positions = cloner.grid_positions(4, spacing=2.0, up_axis=up_axis)
...
def test_replicate_session_authors_every_environment_root(monkeypatch):
    ...
    with cloner.ReplicateSession([cloner.CloneCfg()], 4, 0.5):
        for env_id, position in enumerate(cloner.grid_positions(4, 0.5)):
...
@pytest.mark.parametrize(("replicate_physics", "requires_usd_stage"), [(False, False), (True, True)])
def test_session_registers_one_required_usd_scene_context(monkeypatch, replicate_physics, requires_usd_stage):
    """USD-only cloning and declared scene-data requirements register one scene context."""
    stage = Usd.Stage.CreateInMemory()
    simulation = _simulation(stage=stage)
    simulation.requires_usd_stage = requires_usd_stage
    monkeypatch.setattr(SimulationContext, "instance", lambda: simulation)
    monkeypatch.setattr(replicate_session, "replicate", lambda *args, **kwargs: None)

    with cloner.ReplicateSession([cloner.CloneCfg(replicate_physics=replicate_physics)], 2, 1.0):
        pass
...
def test_plan_routes_cfg_rows_declaratively():
    ...
    assets, names, contexts = _asset_inventory([default_cfg, exact_cfg, empty_cfg])
...
def test_plan_unions_repeated_context_requests_for_one_covering_row():
    ...
    assets, names, contexts = _asset_inventory([child, parent])
    plan = _make_clone_plan(
        cloner.CloneCfg(),
        assets,
        names,
...
def test_replicate_physics_false_skips_only_physics_only_contexts(monkeypatch):
    ...
    assert simulation._clone_models_initialized is False
...
def test_first_hard_reset_dispatches_models_after_stage_edits_once(monkeypatch):
    ...
    calls = []
    model_type = type("Model", (_Context,), {})
    unplanned_type = type("UnplannedModel", (_Context,), {})
    model = model_type("model", calls)
    unplanned = unplanned_type("unplanned", calls)
    simulation = _simulation(
        {model_type: model, unplanned_type: unplanned},
        {model_type: {"physics", "model"}, unplanned_type: {"physics", "model"}},
    )
    plan = _plan(0)
    ...
    assert unplanned.replicated == 0
    assert simulation._clone_models_initialized is True
...
def test_model_failure_makes_clone_lifecycle_terminal():
    """A failed model is never retried, nor are earlier successful models rebuilt."""
    calls = []
    first = _Context("first", calls)
    ...
    simulation = _simulation(
        {type(first): first, type(failing): failing},
        {type(first): {"model"}, type(failing): {"model"}},
    )
    simulation._clone_plan = _plan(0)
    simulation._clone_plan_dispatched = True
    simulation._clone_plan.context_rows.update({type(first): (), type(failing): ()})
    simulation.physics_manager = SimpleNamespace(reset=lambda _soft: pytest.fail("physics reset ran"))
    ...
    assert simulation._clone_models_initialized is None
