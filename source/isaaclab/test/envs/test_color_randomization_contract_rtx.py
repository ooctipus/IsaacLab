...
def test_visual_color_contract_isaac_rtx():
    """Bucket writes propagate to all envs, buckets are distinct and independent on the Isaac RTX renderer."""
    assert_color_contract(IsaacRtxRendererCfg(), physics_cfg=None)  # PhysX is the SimulationCfg default


def test_per_env_visual_color_contract_isaac_rtx():
    """One logical per-env material diverges the envs and honors env_ids on the Isaac RTX renderer."""
    assert_per_env_color_contract(IsaacRtxRendererCfg(), physics_cfg=None)
