...
if not _MISSING_MODULES:
    from color_dr_contract_common import assert_color_contract, assert_per_env_color_contract
    from isaaclab_newton.physics import MJWarpSolverCfg, NewtonCfg
    from isaaclab_newton.renderers import NewtonWarpRendererCfg


def test_visual_color_contract_newton_warp():
    """Bucket writes propagate to all envs, buckets are distinct and independent on the Newton-Warp renderer."""
    assert_color_contract(NewtonWarpRendererCfg(), physics_cfg=NewtonCfg(solver_cfg=MJWarpSolverCfg()))


def test_per_env_visual_color_contract_newton_warp():
    """One logical per-env material diverges the envs and honors env_ids on the Newton-Warp renderer."""
    assert_per_env_color_contract(NewtonWarpRendererCfg(), physics_cfg=NewtonCfg(solver_cfg=MJWarpSolverCfg()))
