...
"""Shared rigid-object rendering contract for renderer backends.

The contract owns the scene and assertions. Backend test modules provide only
the simulation context and renderer configuration needed to run it.
"""
...
def _noop() -> None:
    """Default backend cleanup callback."""
...
class RigidObjectRenderingBackend:
    """Backend hooks and capabilities required by the rendering contract."""

    name: str
    """Human-readable backend name used in assertion diagnostics."""

    simulation_context_factory: Callable[[], AbstractContextManager[SimulationContext]]
    """Create the backend-specific simulation context."""

    renderer_cfg_factory: Callable[[], RendererCfg]
    """Create the renderer configuration assigned to the contract camera."""

    with_articulation: bool = False
    """Whether a lightweight articulation should coexist with the rigid object."""

    cleanup: Callable[[], None] = _noop
    """Release process-global renderer resources after each contract run."""


def _make_scene_cfg(backend: RigidObjectRenderingBackend) -> InteractiveSceneCfg:
    """Create the common scene configuration for one backend contract run."""
    ...
    class _SceneCfg(InteractiveSceneCfg):
        ...
        camera: CameraCfg = CameraCfg(
            ...
            data_types=["depth"],
            renderer_cfg=backend.renderer_cfg_factory(),
...
def run_rigid_object_scale_and_pose_rendering_contract(backend: RigidObjectRenderingBackend) -> None:
    """Run the shared root-scale and pose-synchronization rendering contract."""
    with backend.simulation_context_factory() as sim:
        ...
        try:
            ...
            if backend.with_articulation:
                assert scene["articulation"].is_initialized
