...
def retrieve_file_path(
    ...
    - Local path: returned as-is (absolute).
    - Remote USD: download USD + all dependencies and return local root USD.
    - Other remote files: download a single file and return its local path.
    """
    ...
    if status == 2:
        ...
        if _is_usd_path(url):
            logger.info("Downloading USD file and dependencies from: %s", url)
            mapping = client.download_usd_with_references_sync(
                url,
                download_root=download_dir,
                force_overwrite=force_download,
                progress_callback=lambda done, total, src: logger.info(
            ...
            logger.info("  Main file downloaded to: %s", local_root)
