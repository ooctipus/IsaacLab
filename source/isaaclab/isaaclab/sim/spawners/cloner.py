from pxr import Usd, UsdUtils, Sdf, UsdGeom, PhysxSchema
...
def physx_replicate(
    ...
    current_dests: list[str] = []  # mutated per-row before each replicate() so rename_fn indexes into the right list
    row_world_ids = [torch.nonzero(mapping[i], as_tuple=True)[0].tolist() for i in range(mapping.size(0))]
    row_dests = [[destinations[i].format(w) for w in row_world_ids[i]] for i in range(mapping.size(0))]
