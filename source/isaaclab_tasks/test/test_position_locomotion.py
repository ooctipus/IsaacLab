...
@pytest.mark.parametrize("task_name", ["Isaac-Position-Spot"])
@pytest.mark.parametrize("object", ["all", "gap", "pit"])
...
def test_dexsuite_position_locomotion_terrain_types(task_name: str, object: str, num_envs: int, num_steps: int, device: str):
    # build CLI overrides for Hydra
    sys.argv = [sys.argv[0]]
    sys.argv.append(f"env.scene.terrain={object}")
...
@pytest.mark.parametrize("task_name", ["Isaac-Position-Go2", "Isaac-Position-Anymal-C", "Isaac-Position-Spot"])
@pytest.mark.parametrize("object", ["all"])
@pytest.mark.parametrize("num_envs", [32])
@pytest.mark.parametrize("num_steps", [5])
@pytest.mark.parametrize("device", ["cuda"])  # fabrics sim does not support cpu
def test_dexsuite_position_locomotion_envs(task_name: str, object: str, num_envs: int, num_steps: int, device: str):
    # build CLI overrides for Hydra
    sys.argv = [sys.argv[0]]
    sys.argv.append(f"env.scene.terrain={object}")
    task_name = f"{task_name}-v0"
