...
class CommandPayloadBase:
    ...
    def _init_success_gates(self, cfg: StateCommandCfg, env: ManagerBasedEnv) -> None:
        """Bind the joint-effort and foot-support gates used during successful holds."""
        if self.table.foot_body_ids is None or len(self.table.foot_body_ids) == 0:
            raise ValueError("Command payload success gates require task-table foot_body_ids.")
        ...
        for foot_id in self._foot_ids:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Contact sensor {payload_cfg.contact_sensor_name!r} does not cover foot body {body_name!r}."
                ) from err
        ...
        if self._gravity_magnitude <= 0.0:
            raise ValueError("Command payload success gates require non-zero gravity.")
    ...
    def _success_timer_gate(self) -> torch.Tensor:
        """Return physical gates required while accumulating successful hold time."""
        ...
        effort_threshold = self._success_effort_multiplier / len(self._foot_ids)
        ...
        feet_bear_weight = foot_fz / self._weight >= self._success_min_foot_weight_fraction
