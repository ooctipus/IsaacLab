...
class reset_accumulator(ManagerTermBase):
    """Accumulate validated reset states into a shared state table.

    Reset states are generated and validated during term construction. Runtime
    reset calls only apply the slots selected by the curriculum sampler through
    :attr:`sampled_slots`.
    ...
    def __init__(self, cfg, env):
        ...
        self.state_coords: torch.Tensor | None = None
        self.slot_indices: torch.Tensor | None = None

        self.monitor_success_rate: torch.Tensor | None = None
        self._precollect_state_table(env, cfg.params["reset_term"])
        self._finalize_state_table(env)

    # ------------------------------------------------------------------
    # Main entry point
    # ------------------------------------------------------------------

    def _precollect_state_table(self, env: ManagerBasedRLEnv, reset_term: EventTermCfg) -> None:
        """Precollect the finalized reset-state table."""
    ...
    def _finalize_state_table(self, env: ManagerBasedRLEnv) -> None:
        """Expose bindable tensors consumed by the curriculum sampler."""
        self.state_coords = extract_features(self.state_data, self._state_fps_features).contiguous()
        self.slot_indices = torch.arange(self.state_data.shape[0], device=self.state_data.device, dtype=torch.long)
        self.monitor_success_rate = torch.zeros(self.state_data.shape[0], device=env.device)
    ...
    def __call__(
        ...
        """Apply the slots selected by the curriculum sampler."""
        # The unused config params are still part of the signature because the
        # manager validates event-term params against ``__call__``.
        ...
        if env_ids.numel() > 0:
            self.apply_sampled_slots(env_ids)
    ...
    def apply_sampled_slots(self, env_ids: torch.Tensor) -> None:
        ...
        This is the per-env resample hook consumed by
        :class:`~..sampling.sampling_strategies.ValueShiftSamplingStrategy`:
        the caller writes the desired slot index into :attr:`sampled_slots`
        for ``env_ids`` and then calls this so the chosen stored state is
        written back to the assets before the critic-obs cache is queried.
        """
