...
class CrlRunner:
    """Replay runner for the CRL algorithm's prefill and transition-flush lifecycle.

    Mirrors :class:`OnPolicyRunner` with two CRL-specific differences in
    ``learn()``:

    1. A **prefill phase** collects random data until the replay buffer reaches
       ``min_replay_size`` before any training begins.
    2. ``compute_returns()`` flushes pending replay transitions; ``update()``
       then trains directly from replay.
    """
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False) -> None:
        ...
        2. Calls ``compute_returns`` only as the pending-transition flush boundary.
        ...
        print(f"[CrlRunner] Prefilling buffer to {min_size} timesteps...", flush=True)
        ...
        print("\n[CrlRunner] Buffer ready. Starting training.", flush=True)
