...
def _runtime_asset_types() -> tuple[type[BaseArticulation], type[BaseRigidObject]]:
    """Import backend-independent runtime asset base types only when a live scene is bound."""
    from isaaclab.assets import BaseArticulation, BaseRigidObject

    return BaseArticulation, BaseRigidObject
...
class ResetStateWriter:
    ...
    def __init__(
        ...
        self._env = env
        self._states = states
        self._entities: tuple[BaseArticulation | BaseRigidObject, ...] = self._bind_entities()
    ...
    def _bind_entities(self) -> tuple[BaseArticulation | BaseRigidObject, ...]:
        articulation_type, rigid_object_type = _runtime_asset_types()
        entities: list[BaseArticulation | BaseRigidObject] = []
