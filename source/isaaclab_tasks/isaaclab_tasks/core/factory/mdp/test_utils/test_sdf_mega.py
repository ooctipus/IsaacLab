...
from isaaclab_tasks.manager_based.manipulation.factory_v1.mdp.util.state_ops import (
    ColliderTransform,
    compute_world_point,
    compose_root_mat,
...
_CUBE_FACES = np.array(
    [[0, 2, 1], [0, 3, 2], [4, 5, 6], [4, 6, 7], [0, 5, 4], [0, 1, 5],
     [2, 7, 6], [2, 3, 7], [0, 7, 3], [0, 4, 7], [1, 6, 5], [1, 2, 6]], dtype=np.int32)
...
def _test_compute_world_point_kernel(
    pos: wp.array(dtype=wp.vec3), quat: wp.array(dtype=wp.quat), pt: wp.array(dtype=wp.vec3), out: wp.array(dtype=wp.vec3)
):
...
def _test_query_sdf_kernel(
    ...
    out[0] = query_collider_sdf(query[0], root_pos[0], root_mat[0], root_mat_inv[0], coll[0], mesh_id[0], max_dist, True)
...
class TestComputeWorldPoint:
    ...
    def _run(self, pos, quat, pt):
        ...
        wp.launch(_test_compute_world_point_kernel, dim=1,
...
class TestComposeRootMat:
    ...
    def _run(self, quat, scale):
        out = wp.zeros(1, dtype=wp.mat33, device="cpu")
        wp.launch(_test_compose_root_mat_kernel, dim=1,
...
class TestQueryColliderSdf:
    ...
    def _run(self, query, root_pos, root_mat, root_mat_inv, coll, mesh):
        ...
        wp.launch(_test_query_sdf_kernel, dim=1, inputs=[
            _wp_vec3(*query),
            _wp_vec3(*root_pos),
...
class TestMegaKernel:
    ...
    def test_single_env_single_obstacle_identity(self):
        ...
        wp.launch(get_signed_distance_mega, dim=2, inputs=[
            body_pos, body_quat, body_ids, local_pts, env_ids,
            obs_pos, obs_quat, obs_scale,
            colliders, handles, counts,
            10.0, True, 1, 1, 2, 1, 1,
        ], outputs=[signs], device="cpu")
    ...
    def test_root_scale_world_distance(self):
        ...
        wp.launch(get_signed_distance_mega, dim=1, inputs=[
            body_pos, body_quat, body_ids, local_pts, env_ids,
            obs_pos, obs_quat, obs_scale,
            colliders, handles, counts,
            10.0, True, 1, 1, 1, 1, 1,
