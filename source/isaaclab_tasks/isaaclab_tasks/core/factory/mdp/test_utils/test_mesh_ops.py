...
def _create_mesh_with_mock(prim_type: str, **kwargs) -> trimesh.Trimesh:
    ...
    with patch.dict(sys.modules, {"pxr": MagicMock(), "pxr.UsdGeom": mock_usdgeom}):
        from isaaclab_tasks.manager_based.manipulation.factory_v1.mdp.util.mesh_ops import create_primitive_mesh
        # We need to inject our mock UsdGeom at call time
        import importlib
        import isaaclab_tasks.manager_based.manipulation.factory_v1.mdp.util.mesh_ops as mesh_ops_mod
        original_func = mesh_ops_mod.create_primitive_mesh
