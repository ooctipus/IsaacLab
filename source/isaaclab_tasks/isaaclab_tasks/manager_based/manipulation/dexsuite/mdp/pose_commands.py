    ...
    def _resample_command(self, env_ids: torch.Tensor):
        ...
        if self.sampling_strategy == "uniform":
            self.term_samples[env_ids] = torch.randint(
                0, len(self.terms), (len(env_ids),), device=self.device, dtype=self.term_samples.dtype
            )
        else:
...
class ObjectUniformTableTopCollisionFreePoseCommand(ObjectUniformPoseCommand):
    ...
    def _collect_candidate_command_b(self, num_samples, validator=None):
        ...
        while torch.any(valid_sample_counts < num_samples):
            ...
            pos_candidate_b, quat_candidate_b = (
                self.pose_command_b[valid_env_ids, :3],
                self.pose_command_b[valid_env_ids, 3:],
            )
        ...
        pbar.close()
        print(
            "Min Sample Collected: ",
            torch.amin(valid_sample_counts).item(),
            "Max Sample Collected: ",
            torch.amax(valid_sample_counts).item(),
        )
