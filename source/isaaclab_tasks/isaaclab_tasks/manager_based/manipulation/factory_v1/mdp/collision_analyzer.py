...
from . import utils as factory_util
...
class CollisionAnalyzer:
    ...
    def __init__(self, cfg: CollisionAnalyzerCfg, env: ManagerBasedRLEnv):
        ...
        for i, body_name in enumerate(body_names):
            ...
            local_pts = factory_util.sample_object_point_cloud(
        ...
        for i, obstacle in enumerate(self.obstacles):
            ...
            for prim, p_hash, rel_tf, eid in zip(
                ...
                if p_hash.item() in obs_h.get_warp_mesh_store():
                    ...
                    wp_mesh = factory_util.prim_to_warp_mesh(prim, device=env.device, relative_to_world=False)
    ...
    def __call__(self, env: ManagerBasedRLEnv, env_ids: torch.Tensor):
        pos_w = (
            wp.to_torch(self.asset.data.body_link_pos_w)[env_ids][:, self.body_ids]
            .unsqueeze(2)
            .expand(-1, -1, self.cfg.num_points, 3)
        )
        quat_w = (
            wp.to_torch(self.asset.data.body_link_quat_w)[env_ids][:, self.body_ids]
            .unsqueeze(2)
        ...
        obstacles_pos_w = torch.cat(
            [
                wp.to_torch(obstacle.data.root_pos_w)[env_ids].view(-1, 1, 1, 3).expand(-1, -1, self.cfg.num_points, 3)
        ...
        obstacles_quat_w = torch.cat(
            [
                wp.to_torch(obstacle.data.root_quat_w)[env_ids].view(-1, 1, 1, 4).expand(-1, cloud.shape[1], self.cfg.num_points, 4)
                for obstacle in self.obstacles
            ],
            dim=0,
        )
        ...
        wp.launch(
            factory_util.get_signed_distance,
