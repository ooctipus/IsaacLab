...
try:
    ...
    class FabricAction(ActionTerm):  # type: ignore
        """Stub that raises a helpful error when fabrics-sim is missing."""

        def __init__(self, cfg, env):
            raise ImportError("fabrics_sim is not available. Install it via 'isaaclab -g'")
...
class FabricAction(ActionTerm):  # noqa: F811
    ...
    def __init__(self, cfg: action_cfg.FabricActionCfg, env: ManagerBasedEnv):
        ...
        super().__init__(cfg, env)
        palm_max_angle = [0.0, 0.7, 1.0, -2.4 + cfg.palm_rot_range, cfg.palm_rot_range, 3.14 + cfg.palm_rot_range]
        palm_min_angle = [-1.2, -0.7, 0.0, -2.4 - cfg.palm_rot_range, -cfg.palm_rot_range, 3.14 - cfg.palm_rot_range]
        ...
        self.f_robot_cfg = self.cfg.fabric_robot_scene_cfg
        self.f_robot_cfg.resolve(env.scene)
    ...
    def _setup_geometric_fabrics(self) -> None:
        ...
        self.inputs = [
            ...
            "euler_zyx",  # actions in
            self.fabric_q.detach(),
            self.fabric_qd.detach(),  # fabric state
            self.object_ids,
            self.object_indicator,  # world model
    ...
    def apply_actions(self) -> None:
        ...
        self._asset.set_joint_position_target(self.fabric_q, joint_ids=self.f_robot_cfg.joint_ids)
        self._asset.set_joint_velocity_target(vel_target, joint_ids=self.f_robot_cfg.joint_ids)

    def reset(self, env_ids: Sequence[int] | None):
        self.fabric_q[env_ids] = self._asset.data.joint_pos[env_ids[:, None], self.f_robot_cfg.joint_ids].clone()
        self.fabric_qd[env_ids] = self._asset.data.joint_vel[env_ids[:, None], self.f_robot_cfg.joint_ids].clone()
