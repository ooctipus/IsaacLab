...
def spatial_topk_xy(
    ...
    Args:
        ...
        cell_size: Hash-grid cell edge [m]. ``None`` defaults to
            ``radius`` (the canonical recommendation in ``warp.HashGrid``).
    ...
    if not points_xy.is_cuda or not queries_xy.is_cuda:
        raise ValueError("spatial_topk_xy requires CUDA tensors")

    device = points_xy.device
    n = int(points_xy.shape[0])
    q = int(queries_xy.shape[0])
    if cell_size is None:
        # Slightly larger than the query radius so points that fall on the
        # outer boundary of the radius-ball aren't excluded due to FP
        # rounding at cell edges; the per-point distance filter inside the
        # kernel still enforces the exact ``radius``.
    ...
    # Grid sized for the actual extent. Edges of ``cell_size`` keep
    # neighbour iteration bounded to a 3x3x1 cell shell at the worst.
    grid = wp.HashGrid(
    ...
    wp.launch(
        _topk_query_kernel,
        dim=q,
        inputs=[grid.id, pts_wp, qs_wp, float(radius), int(k)],
