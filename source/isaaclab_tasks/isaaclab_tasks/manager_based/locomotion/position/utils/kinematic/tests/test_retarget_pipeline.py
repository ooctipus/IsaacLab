...
def _make_pipeline(foot_names, max_cand=500):
    return RetargetPipeline(RetargetPipelineCfg(
        kin=KIN_CFG,
...
class TestPipelineStair:
    ...
    def test_stair_acceptance(self, robot_data, stair_mesh):
        kin, foot_ids, foot_names = robot_data
        pipeline = _make_pipeline(foot_names, 1000)
...
class TestPipelineDeterminism:
    ...
    def test_deterministic(self, robot_data):
        ...
        results = []
        for _ in range(2):
            pipeline = _make_pipeline(foot_names, 200)
