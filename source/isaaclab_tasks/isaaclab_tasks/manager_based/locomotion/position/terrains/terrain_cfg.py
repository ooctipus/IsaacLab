...
def _spawn_and_target(**kwargs):
    """Helper to build identical spawn and target flat-patch sampling configs.

    Accepts any keyword arguments for ``FlatPatchSamplingByRadiusCfg`` and returns
    a dict with matching entries for both keys.
    """
    return {
        "spawn": FlatPatchSamplingByRadiusCfg(**kwargs),
        "target": FlatPatchSamplingByRadiusCfg(**kwargs),
    }
...
GAP = terrain_cfg.MeshGapTerrainCfg(
    ...
    flat_patch_sampling=_spawn_and_target(
        num_patches=10,
        patch_radius=0.5,
        radius_range=(0.5, 5.0),
        max_height_diff=0.2,
    ),
)

GAP_PLAY = terrain_cfg.MeshGapTerrainCfg(
    platform_width=3.0,
    gap_width_range=(0.05, 1.5),
    flat_patch_sampling=_spawn_and_target(
        num_patches=10,
...
PIT_PLAY = terrain_cfg.MeshPitTerrainCfg(
    platform_width=3.0,
    pit_depth_range=(0.05, 1.2),
    flat_patch_sampling=_spawn_and_target(
        num_patches=10,
...
RADIATING_BEAM = octilab_terrain.MeshRadiatingBeamTerrainCfg(
    ...
    flat_patch_sampling=_spawn_and_target(
        num_patches=10, patch_radius=0.05, radius_range=(0.5, 5.0), max_height_diff=0.2, z_range=(-1, 1)
    ),
)


RADIATING_BEAM_PLAY = octilab_terrain.MeshRadiatingBeamTerrainCfg(
    platform_width=3.0,
    num_bars=15,
    border_size=(6.5, 6.5),
    bar_width_range=(0.7, 0.3),
    bar_height_range=(1.5, 1.5),
    flat_patch_sampling=_spawn_and_target(
        num_patches=10, patch_radius=0.05, radius_range=(4.0, 5.0), max_height_diff=0.2, z_range=(-1, 1)
    ),
)
...
SQUARE_PILLAR_OBSTACLE = terrain_cfg.HfDiscreteObstaclesTerrainCfg(
    ...
    flat_patch_sampling=_spawn_and_target(
        ...
        patch_radius=0.5,
        max_height_diff=0.2,
        radius_range=(0.5, 6.0),
    ),
)

SQUARE_PILLAR_OBSTACLE_PLAY = terrain_cfg.HfDiscreteObstaclesTerrainCfg(
    num_obstacles=8,
    obstacle_height_mode="fixed",
    obstacle_height_range=(2.0, 3.0),
    obstacle_width_range=(0.5, 1.5),
...
IRREGULAR_PILLAR_OBSTACLE_PLAY = terrain_cfg.MeshRepeatedBoxesTerrainCfg(
    platform_width=2,
    max_height_noise=0.5,
    object_params_start=terrain_cfg.MeshRepeatedBoxesTerrainCfg.ObjectCfg(
        num_objects=5, height=4.0, size=(0.5, 0.5), max_yx_angle=0.0, degrees=True
...
SLOPE_INV = terrain_cfg.HfInvertedPyramidSlopedTerrainCfg(
    ...
    flat_patch_sampling=_spawn_and_target(
        ...
        patch_radius=0.5,
        max_height_diff=0.2,
        radius_range=(0.5, 4.5),
    ),
)

SLOPE_INV_PLAY = terrain_cfg.HfInvertedPyramidSlopedTerrainCfg(
    slope_range=(0.0, 0.9),
    platform_width=2.0,
    border_width=1.5,
    flat_patch_sampling=_spawn_and_target(
...
EXTREME_STAIR_PLAY = terrain_cfg.HfPyramidStairsTerrainCfg(
    platform_width=3.0,
    step_height_range=(0.05, 0.2),
    step_width=0.3,
    inverted=True,
...
STEPPING_STONE_PLAY = octilab_terrain.MeshStonesEverywhereTerrainCfg(
    w_gap=(0.04, 0.26),
    w_stone=(0.96, 0.2),
    s_max=(0.018, 0.118),
    h_max=(0.005, 0.1),
