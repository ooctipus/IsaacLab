...
class CabinetTaskCfg(TaskModuleCfg):
    ...
    def task_obs_terms(self, group: str, robot: RobotModuleCfg) -> dict[str, ObsTerm]:
        ...
        return {
            ...
            "cabinet_handle_error": ObsTerm(
                ...
                params={
                    "ee_frame_cfg": SceneEntityCfg(ee_frame_name, groups=[group]),
                    "cabinet_frame_cfg": SceneEntityCfg(frame_name, groups=[group]),
                },
            ),
        }
    ...
    def reward_terms(self, group: str, robot: RobotModuleCfg) -> dict[str, RewTerm]:
        ...
        frame_name = self._frame_name(robot)
        ee_frame_cfg = SceneEntityCfg(ee_frame_name, groups=[group])
        cab_frame_cfg = SceneEntityCfg(frame_name, groups=[group])
        cab_asset_cfg = SceneEntityCfg(cabinet_name, joint_names=["drawer_top_joint"], groups=[group])
        ...
        return {
            ...
            f"{group}_grasp_handle": RewTerm(
                ...
                params={
                    "threshold": 0.03,
                    "open_joint_pos": 0.04,
                    "asset_cfg": SceneEntityCfg(robot_name, joint_names=[finger_joint_pattern], groups=[group]),
    ...
    def reset_events(self, group: str, robot: RobotModuleCfg) -> dict[str, EventTerm]:
        ...
        return {
            f"{group}_reset_cabinet": EventTerm(
                ...
                mode="reset",
                params={"asset_cfgs": [SceneEntityCfg(cabinet_name, groups=[group])]},
            ),
        }
