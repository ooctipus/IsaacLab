...
* **Scene** — all robot and task assets in per-group
  :class:`~isaaclab.scene.InclusionSet` clone groups.
...
class MultiTaskRegistry:
        ...
        *,
        ...
        Args:
            ...
            physics: Optional physics backend config
                (e.g. :class:`~...config.demo.multitask_physics_cfg.MultitaskPhysicsCfg`).
    ...
    def _build_scene(
        ...
        # ── Clone groups ──────────────────────────────────────────────
        clone_groups: dict[str, InclusionSet] = {}
        for reg in self._regs:
            # Exclude shared (global) robot keys from the clone group so
            # IsaacLab does not try to group-filter a globally-spawned asset.
            ...
            clone_groups[reg.group] = InclusionSet(
                assets=robot_keys + task_keys,
                weight=reg.weight,
            )

        # ── All scene assets ──────────────────────────────────────────
        scene_attrs: dict[str, object] = {
            "clone_cfg": CloneCfg(clone_groups=clone_groups),
            "plane": AssetBaseCfg(
