...
def apply_task_filter(env_cfg: ManagerBasedRLEnvCfg, disabled: set[str]) -> None:
    """Disable clone groups in *env_cfg* in-place.

    Sets each named group to ``weight=0``.  All downstream effects are now
    automatic — no further config patching is required:

    * **Assets** — :class:`~isaaclab.scene.InteractiveSceneCfg.__post_init__`
      nulls asset attributes that belong exclusively to weight-zero groups, so
      :class:`~isaaclab.scene.InteractiveScene` never spawns them.
    ...
    * **Events** — ``layout.filter_reset_ids`` returns empty indices for
      weight-zero groups, causing event functions to return before any asset
      access.
    ...
    Args:
        ...
        disabled: Set of clone-group names to disable.
    """
    if not disabled:
        return

    # ── 1. Mark groups as weight=0 ────────────────────────────────────────
    for name in disabled:
        env_cfg.scene.clone_cfg.clone_groups[name].weight = 0
