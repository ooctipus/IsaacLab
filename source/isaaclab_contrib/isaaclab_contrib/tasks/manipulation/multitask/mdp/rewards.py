...
Each wrapped function computes on ``SceneEntityCfg.group_ids`` rows and returns
...
def object_ee_distance(
    ...
    *,
    ...
    obj_pos = wp.to_torch(env.scene[object_cfg.name].data.root_pos_w)[object_cfg.group_ids]
...
def object_is_lifted(
    ...
    *,
    ...
    height = wp.to_torch(env.scene[object_cfg.name].data.root_pos_w)[object_cfg.group_ids, 2]
...
def object_goal_distance(
    ...
    *,
    ...
    obj_pos = wp.to_torch(rigid_object.data.root_pos_w)[object_cfg.group_ids]
...
def cabinet_align_ee_handle(
    ...
    ee_quat = wp.to_torch(env.scene[ee_frame_cfg.name].data.target_quat_w)[ee_frame_cfg.group_ids, 0, :]
...
def cabinet_align_grasp_around_handle(
    ...
    fingertips = wp.to_torch(env.scene[ee_frame_cfg.name].data.target_pos_w)[ee_frame_cfg.group_ids, 1:, :]
    left = fingertips[:, 0, :]
    right = fingertips[:, 1, :]
...
def cabinet_grasp_handle(
    ...
    *,
    ...
    joint_pos_g = wp.to_torch(robot.data.joint_pos)[asset_cfg.group_ids, :]
...
def cabinet_open_drawer_bonus(
    ...
    drawer_pos = wp.to_torch(cabinet.data.joint_pos)[cabinet_asset_cfg.group_ids, cabinet_asset_cfg.joint_ids].squeeze(
        -1
    )
    handle_pos = wp.to_torch(env.scene[cabinet_frame_cfg.name].data.target_pos_w)[
        cabinet_frame_cfg.group_ids, 0, :
