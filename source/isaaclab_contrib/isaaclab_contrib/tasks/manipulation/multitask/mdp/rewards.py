...
Each wrapped function computes on ``SceneEntityCfg.local_ids`` rows and returns
...
def joint_vel_l2(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg) -> ScatterResult:
    ...
    vel_data = wp.to_torch(robot.data.joint_vel)[asset_cfg.local_ids]
    if asset_cfg.joint_ids != slice(None):
        vel_data = vel_data[:, asset_cfg.joint_ids]
    return asset_cfg.global_ids, torch.sum(vel_data.square(), dim=-1)
...
def position_command_error(
    ...
    root_pos = wp.to_torch(robot.data.root_pos_w)[asset_cfg.local_ids]
    root_quat = wp.to_torch(robot.data.root_quat_w)[asset_cfg.local_ids]
    body_pos = wp.to_torch(robot.data.body_pos_w)[asset_cfg.local_ids, body_idx]
    cmd_pos = env.command_manager.get_command(command_name)[asset_cfg.global_ids, :3]
    des_pos_w, _ = math_utils.combine_frame_transforms(root_pos, root_quat, cmd_pos)
    return asset_cfg.global_ids, torch.linalg.norm(body_pos - des_pos_w, dim=1)
...
def orientation_command_error(
    ...
    body_idx = asset_cfg.body_ids[0]
    root_quat = wp.to_torch(robot.data.root_quat_w)[asset_cfg.local_ids]
    body_quat = wp.to_torch(robot.data.body_quat_w)[asset_cfg.local_ids, body_idx]
    cmd_quat = env.command_manager.get_command(command_name)[asset_cfg.global_ids, 3:7]
    des_quat_w = math_utils.quat_mul(root_quat, cmd_quat)
    return asset_cfg.global_ids, math_utils.quat_error_magnitude(body_quat, des_quat_w)
...
def object_ee_distance(
    ...
    *,
    ...
    obj_pos = wp.to_torch(env.scene[object_cfg.name].data.root_pos_w)[object_cfg.local_ids]
    ee_pos = wp.to_torch(env.scene[ee_frame_cfg.name].data.target_pos_w)[ee_frame_cfg.local_ids, 0, :]
    norm = torch.linalg.norm(obj_pos - ee_pos, dim=1)
    return object_cfg.global_ids, 1.0 - torch.tanh(norm / std)
...
def object_is_lifted(
    ...
    *,
    ...
    height = wp.to_torch(env.scene[object_cfg.name].data.root_pos_w)[object_cfg.local_ids, 2]
    return object_cfg.global_ids, torch.where(height > minimal_height, 1.0, 0.0)
...
def object_goal_distance(
    ...
    *,
    ...
    root_pos = wp.to_torch(robot.data.root_pos_w)[robot_cfg.local_ids]
    root_quat = wp.to_torch(robot.data.root_quat_w)[robot_cfg.local_ids]
    obj_pos = wp.to_torch(rigid_object.data.root_pos_w)[object_cfg.local_ids]
    cmd_pos = env.command_manager.get_command(command_name)[robot_cfg.global_ids, :3]
    des_pos_w, _ = math_utils.combine_frame_transforms(root_pos, root_quat, cmd_pos)
    dist = torch.linalg.norm(des_pos_w - obj_pos, dim=1)
    return robot_cfg.global_ids, (obj_pos[:, 2] > minimal_height) * (1.0 - torch.tanh(dist / std))
...
def cabinet_approach_ee_handle(
    ...
    *,
    ...
    ee_pos = wp.to_torch(env.scene[ee_frame_cfg.name].data.target_pos_w)[ee_frame_cfg.local_ids, 0, :]
    handle_pos = wp.to_torch(env.scene[cabinet_frame_cfg.name].data.target_pos_w)[cabinet_frame_cfg.local_ids, 0, :]
    ...
    return ee_frame_cfg.global_ids, reward
...
def cabinet_align_ee_handle(
    ...
    ee_quat = wp.to_torch(env.scene[ee_frame_cfg.name].data.target_quat_w)[ee_frame_cfg.local_ids, 0, :]
    handle_quat = wp.to_torch(env.scene[cabinet_frame_cfg.name].data.target_quat_w)[cabinet_frame_cfg.local_ids, 0, :]
    ...
    return ee_frame_cfg.global_ids, 0.5 * (torch.sign(align_z) * align_z**2 + torch.sign(align_x) * align_x**2)
...
def cabinet_align_grasp_around_handle(
    ...
    handle_pos = wp.to_torch(env.scene[cabinet_frame_cfg.name].data.target_pos_w)[cabinet_frame_cfg.local_ids, 0, :]
    fingertips = wp.to_torch(env.scene[ee_frame_cfg.name].data.target_pos_w)[ee_frame_cfg.local_ids, 1:, :]
    left = fingertips[:, 0, :]
    right = fingertips[:, 1, :]
    is_grasp = (right[:, 2] < handle_pos[:, 2]) & (left[:, 2] > handle_pos[:, 2])
    return ee_frame_cfg.global_ids, is_grasp.float()
...
def cabinet_approach_gripper_handle(
    ...
    *,
    ...
    return ee_frame_cfg.global_ids, is_graspable * ((offset - left_dist) + (offset - right_dist))
...
def cabinet_grasp_handle(
    ...
    *,
    ...
    joint_pos_g = wp.to_torch(robot.data.joint_pos)[asset_cfg.local_ids, :]
    gripper = torch.sum(open_joint_pos - joint_pos_g[:, joint_ids], dim=-1)
    distance = torch.linalg.norm(handle_pos - ee_pos, dim=-1, ord=2)
    return asset_cfg.global_ids, (distance <= threshold) * gripper
...
def cabinet_open_drawer_bonus(
    ...
    drawer_pos = wp.to_torch(cabinet.data.joint_pos)[cabinet_asset_cfg.local_ids, cabinet_asset_cfg.joint_ids].squeeze(
        -1
    )
    handle_pos = wp.to_torch(env.scene[cabinet_frame_cfg.name].data.target_pos_w)[cabinet_frame_cfg.local_ids, 0, :]
    fingertips = wp.to_torch(env.scene[ee_frame_cfg.name].data.target_pos_w)[ee_frame_cfg.local_ids, 1:, :]
    ...
    return cabinet_asset_cfg.global_ids, (is_graspable + 1.0) * drawer_pos
...
def cabinet_multi_stage_open_drawer(
    ...
    return cabinet_asset_cfg.global_ids, open_easy + open_medium + open_hard
