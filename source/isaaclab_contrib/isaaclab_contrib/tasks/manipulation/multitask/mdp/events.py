...
- ``group_ids``: indices into the asset's data buffer
"""
...
def reset_to_default(
    ...
    *,
    ...
    for asset_cfg in asset_cfgs:
        env_ids_filtered, group_ids = layout.filter_reset_ids(asset_cfg.name, env_ids)
        if env_ids_filtered.numel() == 0:
            continue

        asset = env.scene[asset_cfg.name]
        pose = wp.to_torch(asset.data.default_root_pose)[group_ids].clone()
        vel = wp.to_torch(asset.data.default_root_vel)[group_ids].clone()
        pose[:, :3] += env.scene.env_origins[env_ids_filtered]
        asset.write_root_pose_to_sim_index(root_pose=pose, env_ids=group_ids)
        asset.write_root_velocity_to_sim_index(root_velocity=vel, env_ids=group_ids)

        if hasattr(asset.data, "default_joint_pos"):
            joint_pos = wp.to_torch(asset.data.default_joint_pos)[group_ids].clone()
            joint_vel = wp.to_torch(asset.data.default_joint_vel)[group_ids].clone()
            asset.write_joint_position_to_sim_index(position=joint_pos, env_ids=group_ids)
            asset.write_joint_velocity_to_sim_index(velocity=joint_vel, env_ids=group_ids)
            if reset_joint_targets:
                asset.set_joint_position_target_index(target=joint_pos, env_ids=group_ids)
                asset.set_joint_velocity_target_index(target=joint_vel, env_ids=group_ids)
...
def reset_joints(
    ...
    *,
    ...
    env_ids, group_ids = layout.filter_reset_ids(asset_cfg.name, env_ids)
    if env_ids.numel() == 0:
        return

    robot = env.scene[asset_cfg.name]
    joint_ids = asset_cfg.joint_ids
    env_slice = group_ids[:, None] if joint_ids != slice(None) else group_ids
    ...
    robot.write_joint_position_to_sim_index(position=joint_pos, joint_ids=joint_ids, env_ids=group_ids)
    robot.write_joint_velocity_to_sim_index(velocity=joint_vel, joint_ids=joint_ids, env_ids=group_ids)
...
def reset_object_uniform(
    ...
    *,
    ...
    env_ids, group_ids = layout.filter_reset_ids(object_cfg.name, env_ids)
    if env_ids.numel() == 0:
        return

    rigid_object = env.scene[object_cfg.name]
    pose = wp.to_torch(rigid_object.data.default_root_pose)[group_ids].clone()
    vel = wp.to_torch(rigid_object.data.default_root_vel)[group_ids].clone()
    ranges = torch.tensor(
        [pose_range.get(k, (0.0, 0.0)) for k in ("x", "y", "z", "roll", "pitch", "yaw")],
        device=rigid_object.device,
    )
    samples = math_utils.sample_uniform(ranges[:, 0], ranges[:, 1], (len(group_ids), 6), device=rigid_object.device)
    ...
    vel_samples = math_utils.sample_uniform(
        vel_ranges[:, 0], vel_ranges[:, 1], (len(group_ids), 6), device=rigid_object.device
    )
    vel += vel_samples
    rigid_object.write_root_pose_to_sim_index(root_pose=pose, env_ids=group_ids)
    rigid_object.write_root_velocity_to_sim_index(root_velocity=vel, env_ids=group_ids)
