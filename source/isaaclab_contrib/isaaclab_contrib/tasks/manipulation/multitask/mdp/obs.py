...
asset and command data with ``SceneEntityCfg.local_ids`` / ``env_ids`` and
...
def ee_pose(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg) -> ScatterResult:
    ...
    return asset_cfg.global_ids, torch.cat([pos_b, quat_b], dim=-1)
...
def ee_pos_error(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg, command_name: str = "ee_pose") -> ScatterResult:
    ...
    return asset_cfg.global_ids, cmd_pos - cur_b
...
def object_pos_in_robot_frame(
    ...
    obj_pos = wp.to_torch(rigid_object.data.root_pos_w)[object_cfg.local_ids, :3]
    obj_b, _ = math_utils.subtract_frame_transforms(root_pos, root_quat, obj_pos)
    return robot_cfg.global_ids, obj_b
...
def ee_object_pos_error(
    ...
    root_quat = wp.to_torch(robot.data.root_quat_w)[robot_cfg.local_ids]
    obj_pos = wp.to_torch(rigid_object.data.root_pos_w)[object_cfg.local_ids, :3]
    ee_pos = wp.to_torch(ee.data.target_pos_w)[ee_frame_cfg.local_ids, 0, :]
    obj_b, _ = math_utils.subtract_frame_transforms(root_pos, root_quat, obj_pos)
    ee_b, _ = math_utils.subtract_frame_transforms(root_pos, root_quat, ee_pos)
    return robot_cfg.global_ids, obj_b - ee_b
...
def object_target_pos_error(
    ...
    obj_b, _ = math_utils.subtract_frame_transforms(root_pos, root_quat, obj_pos)
    return robot_cfg.global_ids, cmd_pos - obj_b
...
def joint_pos_rel(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg) -> ScatterResult:
    ...
    robot = env.scene[asset_cfg.name]
    pos_data = wp.to_torch(robot.data.joint_pos)[asset_cfg.local_ids]
    default_data = wp.to_torch(robot.data.default_joint_pos)[asset_cfg.local_ids]
    if asset_cfg.joint_ids != slice(None):
        pos_data = pos_data[:, asset_cfg.joint_ids]
        default_data = default_data[:, asset_cfg.joint_ids]
    return asset_cfg.global_ids, pos_data - default_data
...
def joint_vel(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg) -> ScatterResult:
    ...
    return asset_cfg.global_ids, vel_data
...
def generated_commands(
    ...
    return asset_cfg.global_ids, cmd[asset_cfg.global_ids]
...
def cabinet_joint_pos(env: ManagerBasedRLEnv, cabinet_asset_cfg: SceneEntityCfg) -> ScatterResult:
    ...
    pos_data = wp.to_torch(cabinet.data.joint_pos)[cabinet_asset_cfg.local_ids]
    default_data = wp.to_torch(cabinet.data.default_joint_pos)[cabinet_asset_cfg.local_ids]
    ...
    return cabinet_asset_cfg.global_ids, pos_data - default_data
...
def cabinet_joint_vel(env: ManagerBasedRLEnv, cabinet_asset_cfg: SceneEntityCfg) -> ScatterResult:
    ...
    cabinet = env.scene[cabinet_asset_cfg.name]
    vel_data = wp.to_torch(cabinet.data.joint_vel)[cabinet_asset_cfg.local_ids]
    if cabinet_asset_cfg.joint_ids != slice(None):
        vel_data = vel_data[:, cabinet_asset_cfg.joint_ids]
    return cabinet_asset_cfg.global_ids, vel_data
...
def cabinet_rel_ee_drawer_distance(
    ...
    return ee_frame_cfg.global_ids, handle_pos - ee_pos
...
class multi_task_onehot(ManagerTermBase):
    ...
    def __init__(self, cfg: ManagerTermBaseCfg, env: ManagerBasedRLEnv):
        ...
        for i, name in enumerate(groups):
            ...
            group_idx[group_view.global_ids] = i
