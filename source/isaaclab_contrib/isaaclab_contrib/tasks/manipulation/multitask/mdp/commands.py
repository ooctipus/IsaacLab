...
"""Group-aware pose command term for multi-task environments.

Like the core :class:`UniformPoseCommand` but scoped to a subset of
envs via :class:`SceneEntityCfg` ``groups``.  Only writes to
...
from isaaclab.scene.env_layout import filter_to_group
...
class PoseCommand(CommandTerm):
    """Group-aware uniform pose command generator.

    Generates pose commands for a single robot asset scoped to specific
    clone groups.  The command buffer is ``(num_envs, 7)`` but only the
    rows corresponding to the asset's groups are populated.
    """
    ...
    def __init__(self, cfg: PoseCommandCfg, env: ManagerBasedRLEnv):
        ...
        self._group_layout = group_view.layout
    ...
    def __str__(self) -> str:
        ...
        msg += f"\tGroups: {self.cfg.asset_cfg.groups}\n"
        return msg
